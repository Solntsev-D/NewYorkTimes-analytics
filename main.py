...
def Date_Conversion(release_date): # Функция для преобразования даты к виду, который можно сделать типом datetime
...
for url in url_list:
    ...
    try:
        ...
        urls_articles = driver.find_elements(By.XPATH, '//a[contains(@href, "Position")]')
        
        data = []
        for i in range(0, len(titles_articles)): # Записываем найденные данные в список из словарей Data

            release_date = Date_Conversion(release_dates[i].text)
